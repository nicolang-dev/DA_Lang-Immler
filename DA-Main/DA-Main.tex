...
Zur Auswahl der Hardwarekomponenten des Adapters wird zu aller erst die externe Ausstattung des Adapters überlegt. Das bedeutet praktisch alles, mit dem der klassische Endverbraucher letztendlich zu tun hat. Dann kann der interne Teil, also die Technik dahinter individuell auf die Anforderungen des externen Teils designet und entwickelt werden.
\subsubsection{Auswahl externe Hardware}
Der Adapter soll ein möglichst kompakt konstruiertes und stabiles Gehäuse bekommen. Auf beziehungsweise in diesem wird ein einfacher Taster zur Interaktion angebracht. Mit dem Taster sollen einige Funktionen des Adapters ermöglicht werden. Beispielsweise per Klick, Doppelklick oder kurzem Halten. Da sich die Funktionen des Tasters selbst gering halten werden (Verbindungsvorgang, Ein- und Ausschalten, ...) wird nur ein einziger Taster verwendet, um die Komplexität des Gesamtsystems zu senken. Die weitaus komplizierteren Funktionen sollen alle samt in der Smartphone-Applikation ermöglicht werden. Zusätzlich werden eine oder mehrere LED-Leuchten zur Statusanzeige verbaut, um beispielsweise den aktuellen Verbindungsstatus zum Mobilgerät und zum Internet oder den aktuellen Akkustand anzuzeigen. \newline \\
\textbf{Gehäuse} \newline
...
\textbf{Taster} \newline
Für den Taster wird ein herkömmlicher Button in das Gehäuse eingelassen.  \newline \\
\textbf{LED (Light-Emitting Diode)} \newline
Es ist durchaus bekannt, dass LED-Leuchten im Vergleich zu anderer Lichttechnik effizienter und langlebiger sind. \newline
Laut \newline
...
\hline
\end{tabularx} \newline \\
Für dieses Projekt bedeutet das, dass die Leuchtdiode wohl die effizienteste Methode für eine Statusanzeige auf dem Adapter ist.
